...
MainWindow::MainWindow(QWidget *parent)
  ...
  settings = new QSettings("21_school", "3d_Viewer", this);
  timer = new QTimer(0);
...
MainWindow::~MainWindow() {
  save_settings();
  FreeMemory(&ui->openGLWidget->file_data);
  delete settings;
  delete ui;
}
...
void MainWindow::on_select_obj_clicked() {
  ...
  ui->obj_path->setText(QString_filename);
}
...
void MainWindow::on_render_clicked() {
  ...
  std::string std_filename = ui->obj_path->text().toStdString();
  ui->openGLWidget->filename = (char *)std_filename.c_str();
  ui->openGLWidget->parse_obj();
  obj_loaded = true;

  ui->filename_info->setText(QFileInfo(ui->obj_path->text()).fileName());
  ui->edges_info->setText(
      QString::number(ui->openGLWidget->file_data.count_edges));
  ui->vertices_info->setText(
      QString::number(ui->openGLWidget->file_data.count_vertex));
}

void MainWindow::on_projection_type_currentIndexChanged(int index) {
  ui->openGLWidget->projection_type = index;
...
void MainWindow::on_x_move_spinbox_valueChanged(int arg1) {
  if (arg1 > 0 && arg1 < 100) {
    double step = 5;
    if (arg1 - x_move_curr < 0) {
      step *= -1;
    }
    double val = step / ui->openGLWidget->scale_val;
    move_x(&ui->openGLWidget->file_data,
           val * ui->openGLWidget->normalized_coeff / 50.0);
    x_move_curr = arg1;
    ui->openGLWidget->update();
  }
}
...
void MainWindow::on_y_move_spinbox_valueChanged(int arg1) {
  if (arg1 > 0 && arg1 < 100) {
    double step = 5;
    if (arg1 - y_move_curr < 0) {
      step *= -1;
    }
    double val = step / ui->openGLWidget->scale_val;
    move_y(&ui->openGLWidget->file_data,
           val * ui->openGLWidget->normalized_coeff / 50.0);
    y_move_curr = arg1;
    ui->openGLWidget->update();
  }
...
void MainWindow::on_z_move_spinbox_valueChanged(int arg1) {
  if (arg1 > 0 && arg1 < 100) {
    double step = 5;
    if (arg1 - z_move_curr < 0) {
      step *= -1;
    }
    double val = step / ui->openGLWidget->scale_val;
    move_z(&ui->openGLWidget->file_data,
           val * ui->openGLWidget->normalized_coeff / 50.0);
    z_move_curr = arg1;
    ui->openGLWidget->update();
  }
...
void MainWindow::on_x_turn_spinbox_valueChanged(int arg1) {
  if (arg1 > 0 && arg1 < 100) {
    double step = 50;
    if (arg1 - x_turn_curr < 0) {
      step *= -1;
    }
    double val = step / ui->openGLWidget->scale_val;
    turn_around_x(&ui->openGLWidget->file_data,
                  val * ui->openGLWidget->normalized_coeff / 10.0);
    x_turn_curr = arg1;
    ui->openGLWidget->update();
  }
}
...
void MainWindow::on_y_turn_spinbox_valueChanged(int arg1) {
  if (arg1 > 0 && arg1 < 100) {
    double step = 50;
    if (arg1 - y_turn_curr < 0) {
      step *= -1;
    }
    double val = step / ui->openGLWidget->scale_val;
    turn_around_y(&ui->openGLWidget->file_data,
                  val * ui->openGLWidget->normalized_coeff / 10.0);
    y_turn_curr = arg1;
    ui->openGLWidget->update();
  }
...
void MainWindow::on_z_turn_spinbox_valueChanged(int arg1) {
  if (arg1 > 0 && arg1 < 100) {
    double step = 50;
    if (arg1 - z_turn_curr < 0) {
      step *= -1;
    }
    double val = step / ui->openGLWidget->scale_val;
    turn_around_z(&ui->openGLWidget->file_data,
                  val * ui->openGLWidget->normalized_coeff / 10.0);
    z_turn_curr = arg1;
    ui->openGLWidget->update();
  }
...
void MainWindow::on_scale_spinbox_valueChanged(int arg1) {
  if (arg1 > 0 && arg1 < 100) {
    int direction = 0;
    double step = ui->openGLWidget->normalized_coeff / 10;
    double scale_temp = scale_curr;
    if (scale_curr > arg1) {
      direction = -1;
    } else if (scale_curr < arg1) {
      direction = 1;
    }
    scale_curr = arg1;

    if ((int)(scale_curr + direction * step) > 0) {
      scale_curr += direction * step;
      scaling(&ui->openGLWidget->file_data, scale_curr / scale_temp);
      ui->openGLWidget->update();
    }
  }
}
